...
async def fetch_both_directions() -> dict:
    async with aiohttp.ClientSession() as session:
        # Geocode both addresses once (cached after first call)
        (o_lon, o_lat), (d_lon, d_lat) = await asyncio.gather(
            _geocode(session, ORIGIN),
            _geocode(session, DESTINATION),
        )

        outbound, inbound = await asyncio.gather(
            _fetch_direction(session, ORIGIN, DESTINATION, o_lon, o_lat, d_lon, d_lat),
            _fetch_direction(session, DESTINATION, ORIGIN, d_lon, d_lat, o_lon, o_lat),
        )
